...
class About extends React.Component {
    ...
    render() {
        ...
        if (this.props.settings.__selectedProfile && this.props.profiles.hasOwnProperty(this.props.settings.__selectedProfile)){
          ...
          if (aboutFile) {
            machineAbout=<div dangerouslySetInnerHTML={{__html: marked.parse(require('../data/lw.machines/machines/'+aboutFile))}}></div>
          }
        }
...
class Lifesaver extends React.Component
{
    ...
    {
        let button;
        if (window.require) button = <Button size="sm" variant="warning" onClick={(e) => { this.props.handleDevTools(e) }}><Icon name="gear" /> Toggle Dev tools</Button>
        ...
        return <ButtonToolbar>
          ...
          <Button size="sm" variant="warning" onClick={(e) => { this.props.handleRefresh(e) }}><Icon name="refresh" /> Refresh window</Button>
          <Button size="sm" variant="danger" onClick={(e) => { this.props.handleReset(e) }}><Icon name="bolt" /> Reset to factory defaults</Button>
        </ButtonToolbar>
    }
}
...
export class Releases extends React.Component {
    ...
    render() {
        return <div className="releases">
            ...
            {this.state.body ? <div dangerouslySetInnerHTML={{__html: marked.parse(this.state.body)}}/>:undefined }
        </div>
    }
